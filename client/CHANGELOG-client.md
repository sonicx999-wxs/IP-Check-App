...
#### 多页应用架构
- **实现多页应用**: 从单页应用重构为多页应用架构
- **新增门户主页**: 创建 `index.html` 作为项目入口，提供简洁的启动界面
- **页面拆分**: 
  - `app.html`: 检测工具核心页面
  - `guide.html`: 使用文档页面
...
#### 统一导航与样式
- **统一导航栏**: 所有页面添加一致的导航栏设计
- **固定顶部**: 导航栏固定在页面顶部，提升用户体验
- **玻璃拟态风格**: 采用 Glassmorphism 半透明背景设计
- **当前页高亮**: 自动高亮显示当前访问的页面链接

#### 版本号统一管理
- **集中配置**: 创建 `config.js` 文件，统一管理版本号
- **动态加载**: 所有页面和脚本从配置文件读取版本号
- **简化更新**: 修改版本号只需更新配置文件，无需修改多个页面

### 📁 项目结构优化
- **文件重命名**: 将原 `index.html` 重命名为 `app.html`
- **配置文件**: 新增 `config.js` 用于版本管理
- **页面规范**: 统一所有页面的代码结构和样式

### 🐛 修复与优化
- **JS 安全性**: 增强脚本安全性，避免在非工具页报错
- **事件监听**: 优化事件监听器绑定，仅在必要时执行
- **版本水印**: 更新控制台版本水印，使用统一版本号
