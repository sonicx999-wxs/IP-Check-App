...
async function fetchIPQS(ip) {
    ...
    try {
        const url = `https://ipqualityscore.com/api/json/ip/${apiKeys.ipqs}/${ip}?strictness=1`;
...
async function fetchScamalytics(ip) {
    ...
    try {
        // Note: Scamalytics might have strict CORS. 
        // If this fails in browser, we might need a proxy or user needs a CORS plugin.
        const url = `https://api11.scamalytics.com/v3/${apiKeys.scamUser}/?key=${apiKeys.scamKey}&ip=${ip}`;
        const response = await fetch(url);
